            ...
            className={`
                ...
                ${isSuccess 
                    ...
                    : isError
                        ...
                        : 'border-gray-300 bg-white hover:border-[#f6b034] hover:bg-[#000040]/5'
                }
            `}
        >
            {/* Background Icon Decoration */}
            {icon && (
                <div className={`absolute -right-4 -bottom-4 opacity-[0.07] transform rotate-12 transition-transform group-hover:scale-110 group-hover:rotate-6 ${isSuccess ? 'text-green-600' : 'text-[#000040]'}`}>
            ...
            <div className="relative z-10 flex flex-col items-center gap-2 w-full">
                {isLoading ? (
                    <div className="flex flex-col items-center">
                        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#000040] mb-2"></div>
                        <span className="text-xs text-[#000040] font-semibold animate-pulse">Procesando...</span>
                    </div>
                ) : isSuccess ? (
                    ...
                    <div className="flex flex-col items-center group-hover:-translate-y-1 transition-transform duration-300">
                        {icon ? (
                            <div className="rounded-xl p-3 bg-slate-50 text-[#000040] mb-2 group-hover:bg-[#f6b034]/10 group-hover:text-[#f6b034] transition-colors duration-300">
                                {React.cloneElement(icon as React.ReactElement<any>, { className: "w-6 h-6" })}
                            </div>
                        ) : null}

                        <h4 className="font-bold text-sm text-slate-700 group-hover:text-[#000040] transition-colors">{title}</h4>
